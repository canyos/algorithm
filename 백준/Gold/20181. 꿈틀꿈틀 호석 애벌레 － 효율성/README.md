...
1. 배열의 길이가 10만이므로 n^2으로는 해결할 수 없음
2. 애벌레는 연속구간을 먹으므로 투포인터를 사용할 수 있음
3. 연속되는 구간의 만족도를 전부 구한 후 겹치지 않게 최대값을 구하면 됨
4. 투포인터를 사용해 {start,end,value}값을 포함하는 stick을 전부 구한다.
5. end값을 기준으로 dp에서 dp[i] = max(dp[i-1], i번째를 포함하는 경우) 이다.
6. i번째를 포함하는 경우 최대값은 i번째로 끝나는 stick 중 하나를 선택해 dp[stick.start-1]+stick.value의 최대값을 구하면 된다.
7. 기대값은 최대 (1억-1)의 값을 가지고 10만번 누적될 수 있으므로 long long자료형이 필요함.
