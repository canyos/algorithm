...
### 출력 
 ...
 ### 해설
 1. 첫번째 들어온 숫자는 무조건 root가 된다.
 2. 2번째 들어온 숫자는 1번째의 자식이 되야하므로 무조건 qu에 넣는다.
 3. 그 후 i번째 숫자는 i-1번째 숫자와 이어지면 같은 부모를 가지므로 node에 같이 넣어준다.
 4. 모든 자식을 연결했다면 k의 부모의 부모(조부)를 찾는다.
 5. 조부의 자식들에 대해 자식이 k의 부모가 아니라면 손주의 개수를 더한다.
