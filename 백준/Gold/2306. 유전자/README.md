...
### 해설
1. dp[i][j]에는 i부터 j까지의 KOI유전자 길이 저장
2. 만약 str[i]와 str[j]가 같다면 dp[i][j]는 dp[i+1][j-1]+2고려
3. dp[i][j]는 dp[i][k] + dp[k+1][j] 고려
4. j-i는 1인거부터 차례대로 올라가야함
