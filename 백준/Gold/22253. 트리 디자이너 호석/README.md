...
### 해설
1. dp를 통해 자식의 숫자를 이용해야겠다고 생각함
2. 점화식을 찾으려 했지만 찾지 못함
3. 해설봄
4. dy[i][j]는 i번 정점에서 j로 끝나는 수열의 개수
5. dy[i][arr[i]]는 1로 초기화
6. i번 정점을 켜는 경우-> dy[x][arr[x]] = arr[x]~9까지 dy[y][k]의 합 x는 본인 y는 자식 k는 arr[x]~9까지
7. i번 정점을 켜지 않는 경우 -> dx[x][arr[x]] = 0~9까지 dy[x][k] += dy[y][k]
8. 모든 조합의 개수는 0~9까지 dx[1][k]의 합
