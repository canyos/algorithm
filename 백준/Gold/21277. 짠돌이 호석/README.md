...
### 해설
1. 첫 번째 그림을 고정하고 두번째 그림을 회전,이동하면서 알맞은 데이터가 있으면 result를 갱신한다.
2. 첫 번째 그림을 고정했으므로 첫 번째 그림의 위치를 돌면서 두번째 그림의 회전,이동한 그림이 1로 겹치는지 확인한다.
3. 겹친다면 실패
4. 겹치는 부분이 하나도 없다면 성공
5. 성공일 경우 최소 너비, 높이를 구한다. 최소 너비는 max(두 상자중 오른쪽 모서리) - min(두 상자중 왼쪽모서리)이다.
