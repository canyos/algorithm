...
### 해설
1. dp[i][j]에는 i번째 돌에 j개의 돌을 건너뛰어 도착한 최소 점프 횟수
2. dp[i][j] = dp[i-j+1][j-1], dp[i-j][j], dp[i-j-1][j+1]에서 점프한 것 중 최소이다.
3. n번째 돌에 최소 필요한 점프 횟수는 dp[n][k] 중 최소값
