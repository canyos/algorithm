...
### 해설
1. graph를 통해 이어진 edge들을 입력받는다.
2. 3중 반복문으로 플로이드 워셜 알고리즘을 통해 i->j 와 j->k 가 true 이면 i->k도 true로 체크한다.
3. 2중 반복문으로 i->j가 true이면 i->j가 가능하므로 cnt증가시킴
4. 노드의 수는 52개이고 플로이드 워셜 알고리즘의 시간복잡도는 n^3이므로 충분히 시간안에 가능함
